        ...
        <nav className="flex flex-1 flex-col gap-1.5">
          {NAV_ITEMS.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `relative flex items-center gap-2.5 rounded-lg border px-3 py-2.5 font-display text-[11px] font-medium uppercase tracking-wider transition-all ${
                  isActive
                    ? 'border-cyan-400/40 bg-cyan-400/10 text-cyan-200 shadow-[0_0_18px_-6px_color-mix(in_srgb,var(--color-accent)_70%,transparent)]'
            ...
            >
              {({ isActive }) => (
                <>
                  {isActive && <span className="absolute left-0 top-1/2 h-4 w-0.5 -translate-y-1/2 rounded bg-cyan-400 shadow-[0_0_8px_2px_color-mix(in_srgb,var(--color-accent)_80%,transparent)]" />}
